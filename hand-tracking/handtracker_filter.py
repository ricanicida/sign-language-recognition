...
class HandTracker():
    def __init__(
            self, video_fps: int=0, hei_sampling_rate: int=0,
            hei_frame_step: int=1, hei_max_duration: float=0,
            hei_overlap: float=0, hei_max_frames: int=30,
            mode: str='IMAGE', max_hands: int=2,
            detection_con: float=0.5, model_complexity: int=1,
            track_con: float=0.5, apply_thresholding: bool=False,
            grayscale_output: bool=False, apply_gaussian_blur: bool=True
    ) -> None:
        self.mode = mode
    ...
    def adaptive_thresholding(self, image):
        ...
        if self.apply_thresholding:
            ...
            # Convert single channel threshold mask to 3-channel
            mask_3ch = cv2.merge([thresh] * 3)

            # Composite: keep foreground, replace background
            foreground = cv2.bitwise_and(image, mask_3ch)
            background_mask = cv2.bitwise_not(mask_3ch)
            background = np.full_like(image, background_color)
            background = cv2.bitwise_and(background, background_mask)

            result = cv2.add(foreground, background)
        else:
            result = image

        # Return grayscale if flag is set
                ...
                for i in range(self.buffer_length):
                    if len(images[i]) > 0 and -1 not in coordinates[i]:
                        if len(avg_image) == 0:
                            ...
                            filtered_image = self.adaptive_thresholding(cropped_image)
                            filtered_image = self.gaussian_filter(filtered_image)
                            avg_image = filtered_image
                            k += 1
                        else:
                            cx, cy = coordinates[i]
                            cropped_image = self.crop_square(images[i], cx, cy, min_common_box_size)
                            ...
                            beta = 1.0 - alpha
                            try:
                                avg_image = cv2.addWeighted(filtered_image, alpha, avg_image, beta, 0.0)
                            except:
